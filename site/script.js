document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateUI = (seconds) => {
    ...
    elements.progressHandle.setAttribute(
      'transform',
  ...
  const loadAndPlayMusic = () => {
    if (SETTINGS.TRACKS.length > 0) {
      ...
      elements.musicPlayer
        .play()
        .catch((e) => console.error('Müzik Oynatma Hatası:', e));
    }
  };
  ...
  const startTimer = () => {
    ...
    if (state.currentMode === 'work') {
      if (elements.musicPlayer.paused && elements.musicPlayer.currentTime > 0) {
        elements.musicPlayer
          .play()
          .catch((e) => console.error('Müzik devam ettirilemedi:', e));
      } else {
        loadAndPlayMusic();
      }
    }
  ...
  const switchMode = () => {
    ...
    if (state.currentMode !== 'work') {
      ...
      state.currentMode =
        state.pomodoroCount % 4 === 0 ? 'longBreak' : 'shortBreak';
      state.totalTime =
  ...
  const toggleDarkMode = () => {
    const theme = elements.htmlEl.getAttribute('data-theme');
  ...
  elements.progressHandle.addEventListener('touchstart', startDrag, {
    passive: false,
  });
  elements.musicPlayer.addEventListener('ended', () => {
    state.currentTrackIndex =
      (state.currentTrackIndex + 1) % SETTINGS.TRACKS.length;
    loadAndPlayMusic();
  });
